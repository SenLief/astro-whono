...
## Added
- `/archive/` 与 `/essay/` 新增标签浏览入口，并提供支持分页的静态标签结果页。
- 首页导语配置新增“标签”入口，可从首页直接进入归档页并使用标签筛选。
- `/bits/` 新增年份筛选，并增强搜索结果展示，支持关键词高亮、命中片段与首图缩略图。

## Changed
- 列表搜索现可与标签结果页配合使用；归档页搜索时会自动隐藏无结果的年份分组。
- `/archive/` 与 `/essay/` 的列表页、标签页和分页页补充了更明确的页面描述，便于搜索引擎与分享场景识别当前页面内容。
- `/bits/` 改为分页浏览；搜索结果不再受当前分页限制，并可直接跳转到对应内容位置。
- `/bits/` 年份筛选仅显示已有内容的年份；桌面与平板默认展示“全部 + 最近 2 个年份 + 更多”，移动端可直接选择全部已有年份。
- `/bits/` 搜索补充了键盘交互，支持回车搜索、Esc 清空，以及在输入框与结果之间快速切换焦点。

## Fixed
- 修复关闭 JavaScript 时 `/archive/` 与 `/essay/` 标签入口不可用的问题，当前会回退为可直接访问的标签列表。
